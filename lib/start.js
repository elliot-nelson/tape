...
function help() {
    ...
    console.error('  -t      Trace (log every instruction executed)');
    console.error('');
}
...
function main(args) {
    ...
    const tape = new Tape(tapefile.program, { trace: options.trace || false });
    try {
        tape.run();
    } catch (e) {
        if (e instanceof TapeError) {
            console.error('Trace:');
            for (const entry of tape.history || []) {
                console.error(TapeFormatter.formatHistoryEntry(entry));
            }
